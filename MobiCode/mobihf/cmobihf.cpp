/**********************************************************************
 * cmobihf.cpp  C implementation of simple math functions in mobihf.py
 * mostly taken from PyQuante, modified for S60
 * (c) V.Ganesh, 2006, Licence: GPL
 **********************************************************************/
...
static double Bfunc(int i, int r, double g){
  return fact_ratio2(i,r)*mpow(4*g,r-i);
}
...
static double coulomb_repulsion(double xa, double ya, double za, double norma,
  ...
  return 2.*mpow(M_PI,2.5)/(gamma1*gamma2*sqrt(gamma1+gamma2))
...
static double B_term(int i1, int i2, int r1, int r2, int u, int l1, int l2,
  ...
  return fB(i1,l1,l2,Px,Ax,Bx,r1,gamma1)
    *mpow(-1,i2)*fB(i2,l3,l4,Qx,Cx,Dx,r2,gamma2)
    *mpow(-1,u)*fact_ratio2(i1+i2-2*(r1+r2),u)
    *mpow(Qx-Px,i1+i2-2*(r1+r2)-2*u)
    /mpow(delta,i1+i2-2*(r1+r2)-u);
}
...
static double overlap(double alpha1, int l1, int m1, int n1,
  ...
  printf("Called1\n");
  pre = mpow(M_PI/gamma,1.5)*exp(-alpha1*alpha2*rab2/gamma);
  printf("Called2\n");
...
static double A_term(int i, int r, int u, int l1, int l2,
  ...
  /* THO eq. 2.18 */
  return mpow(-1,i)*binomial_prefactor(i,l1,l2,PAx,PBx)*
    mpow(-1,u)*fact(i)*mpow(CPx,i-2*r-2*u)*
    mpow(0.25/gamma,r+u)/fact(r)/fact(u)/fact(i-2*r-2*u);
}
...
static double binomial_prefactor(int s, int ia, int ib, double xpa, double xpb){
  ...
  for (t=0; t<s+1; t++)
    if ((s-ia <= t) && (t <= ib)) 
      sum += binomial(ia,s-t)*binomial(ib,t)*mpow(xpa,ia-s+t)*mpow(xpb,ib-t);
  return sum;
} 
...
static double three_center_1D(double xi, int ai, double alphai,
  ...
  dx = exp(-alphai*alphaj*mpow(xi-xj,2)/gamma) *
    exp(-alphai*alphak*mpow(xi-xk,2)/gamma) *
    exp(-alphaj*alphak*mpow(xj-xk,2)/gamma);
  ...
  for (q=0; q<ai+1; q++){
	...
	if ((q+r+s)%2 == 0) {
	  intgl += binomial(ai,q)*binomial(aj,r)*binomial(ak,s)*
	    mpow(xpi,ai-q)*mpow(xpj,aj-r)*mpow(xpk,ak-s)*
	    fact2(2*n-1)/mpow(2*gamma,n)*sqrt(M_PI/gamma);
	}
      }
    }
  }
...
/* Python interface */
static const PyMethodDef cints_methods[] = {
...
extern "C" {
DL_EXPORT(void) initcints(void){ 
  Py_InitModule("cmobihf", (PyMethodDef*) cints_methods);
}
}
